...
type ManagerHandlers struct {
	...
	cardRequestBuilder func(*gin.Context, *models.Server, interface{}, string) (*cards.Request, gin.H)
}
...
func (h *ManagerHandlers) buildServerStatusCardRequest(c *gin.Context, s *models.Server, username interface{}, errMsg string) (*cards.Request, gin.H) {
	if s == nil {
		return nil, nil
	}
	...
	mgr := h.manager
	datasets := cards.NewDatasets(mgr, s)

	// Sort live/history clients and expose ban metadata via shared datasets so other cards can reuse it.
	var liveSorted []*models.Client
	var historySorted []*models.Client
	var bannedEntries []models.BannedEntry
	var bannedIDs []string
	if datasets != nil {
		if roster := datasets.PlayerRoster(); roster != nil {
			liveSorted = roster.LiveClients
			historySorted = roster.HistoryClients
			bannedEntries = roster.BannedEntries
			bannedIDs = roster.BannedIDs
		}
	}
	if liveSorted == nil {
	...
	if bannedEntries == nil {
		bannedEntries = s.BannedEntries()
	}
	if bannedIDs == nil {
		bannedIDs = s.ReadBlacklistIDs()
	}
	...
	if datasets != nil {
		if summary := datasets.ServerSummary(); summary != nil {
			worldInfo = summary.WorldInfo
			resolvedWorldID = summary.ResolvedWorldID
			startLocationInfo = summary.StartLocationInfo
			startConditionInfo = summary.StartConditionInfo
			languageOptions = summary.LanguageOptions
		}
	}
	if strings.TrimSpace(worldInfo.ID) == "" && mgr != nil {
		worldInfo = mgr.GetWorldInfoWithLanguage(s.World, s.Beta, s.Language)
	}
	if resolvedWorldID == "" && mgr != nil {
		resolvedWorldID = mgr.ResolveWorldID(s.WorldID, s.Beta)
	}
	if startLocationInfo == nil && mgr != nil {
		if locations := mgr.GetStartLocationsForWorldVersionWithLanguage(s.WorldID, s.Beta, s.Language); len(locations) > 0 {
			match := locations[0]
	...
	if startConditionInfo == nil && mgr != nil {
		if conditions := mgr.GetStartConditionsForWorldVersionWithLanguage(s.WorldID, s.Beta, s.Language); len(conditions) > 0 {
			match := conditions[0]
	...
	if len(languageOptions) == 0 && mgr != nil {
		languageOptions = mgr.GetLanguagesForVersion(s.Beta)
	}
	if len(languageOptions) == 0 && mgr != nil {
	...
	if datasets != nil {
		if config := datasets.ServerConfig(); config != nil {
			worldIDs = config.WorldIDs
			worldDisplays = config.Worlds
			worldData = config.WorldData
			releaseDifficulties = config.ReleaseDifficulties
			betaDifficulties = config.BetaDifficulties
			releaseLanguages = config.ReleaseLanguages
			betaLanguages = config.BetaLanguages
		}
	}
	if worldIDs == nil && mgr != nil {
	...
	if len(releaseDifficulties) == 0 && mgr != nil {
		releaseDifficulties = mgr.GetDifficultiesForVersionWithLanguage(false, s.Language)
	}
	if len(betaDifficulties) == 0 && mgr != nil {
		betaDifficulties = mgr.GetDifficultiesForVersionWithLanguage(true, s.Language)
	}
	if len(releaseLanguages) == 0 && mgr != nil {
		releaseLanguages = mgr.GetLanguagesForVersion(false)
	}
	if len(betaLanguages) == 0 && mgr != nil {
		betaLanguages = mgr.GetLanguagesForVersion(true)
	}
	...
	if mgr != nil {
		...
		accessible := make([]int, 0, len(mgr.Servers))
		var all bool
		...
		if prevID > 0 {
			if ps := mgr.ServerByID(prevID); ps != nil {
				c.Set("prev_server_name", ps.Name)
			}
		}
		if nextID > 0 {
			if ns := mgr.ServerByID(nextID); ns != nil {
	...
	serverPath := ""
	if mgr != nil && mgr.Paths != nil {
		serverPath = mgr.Paths.ServerDir(s.ID)
	}
	...
	payload := gin.H{
		...
		"release_difficulties": releaseDifficulties,
		"beta_difficulties":    betaDifficulties,
		// Per-channel languages for per-server selection
		"release_languages":    releaseLanguages,
		"beta_languages":       betaLanguages,
		"language_options":     languageOptions,
		"serverPath":           serverPath,
		"banned":               bannedEntries,
		"banned_ids":           bannedIDs,
		"scon_port":            sconPort,
		"scon_url":             sconURL,
		"start_location_info":  startLocationInfo,
		"start_condition_info": startConditionInfo,
		"card_toggle_options":  cards.ToggleableCardsForScreen(cards.ScreenServerStatus),
	}
	...
	return cardReq, payload
}

func (h *ManagerHandlers) buildManagerPagePayload(c *gin.Context) gin.H {
	if h.manager == nil {
		return nil
	}
...
func (h *ManagerHandlers) buildManagerCardRequest(c *gin.Context) (*cards.Request, gin.H) {
	payload := h.buildManagerPagePayload(c)
	if payload == nil {
		return nil, nil
	}
	req := &cards.Request{
...
func (h *ManagerHandlers) buildDashboardPayload(c *gin.Context) gin.H {
	if h.manager == nil {
		return nil
	}
	user, _ := c.Get("user")
	...
	systemHealthPercent := h.manager.SystemHealthPercent()
	healthScore := "Offline"
	healthPill := "Offline"
	if systemHealthPercent > 0 {
		healthScore = fmt.Sprintf("%.0f%%", systemHealthPercent)
	}
	...
	var userStats gin.H
	if strings.EqualFold(role, "admin") && h.userStore != nil {
		users := h.userStore.Users()
		totalUsers := len(users)
		adminCount := 0
		operatorCount := 0
		for _, usr := range users {
			roleValue := strings.ToLower(strings.TrimSpace(string(usr.Role)))
			switch roleValue {
			case "admin":
				adminCount++
			case "operator":
...
func (h *ManagerHandlers) buildManagerStatusPayload(rootPath string) gin.H {
	status := gin.H{
		"port":                h.manager.Port,
		...
		"status":              "unknown",
		"components_total":    0,
		"components_uptodate": 0,
		"components_outdated": 0,
	}
	if h.manager == nil {
		return status
	}
	status["updating"] = h.manager.IsUpdating()
	total, healthy, outdated := h.managerComponentHealth()
	status["components_total"] = total
	status["components_uptodate"] = healthy
	status["components_outdated"] = outdated
	return status
}

func (h *ManagerHandlers) buildManagerCardContext(status gin.H) gin.H {
	ctx := gin.H{}
	if status == nil {
		status = gin.H{}
	}
	port := toInt(status["port"])
	rootPath := strings.TrimSpace(toString(status["root_path"]))
	updating := toBool(status["updating"])
	...
	if total == 0 && healthy == 0 {
		...
		if outdated == 0 {
			outdated = fallbackOutdated
		}
	}
	pillClass, statusLabel, statusMeta := summarizeManagerStatus(updating, total, healthy)
	now := time.Now()
	ctx["port"] = port
	ctx["port_label"] = formatPortLabel(port)
	ctx["root_path"] = rootPath
	ctx["root_label"] = formatRootLabel(rootPath)
	...
	ctx["status_meta_hidden"] = strings.TrimSpace(statusMeta) == ""
	ctx["timestamp"] = now
	ctx["timestamp_iso"] = now.UTC().Format(time.RFC3339)
	return ctx
}

func summarizeManagerStatus(updating bool, total, healthy int) (string, string, string) {
	if updating {
		return "is-info", "Updating…", ""
	}
	if total <= 0 {
		return "is-warning", "Status Unknown", ""
	}
	if healthy <= 0 {
		return "is-critical", "No software up to date", fmt.Sprintf("%d/%d healthy", healthy, total)
	}
	if healthy < total {
		return "is-warning", "Updates required", fmt.Sprintf("%d/%d healthy", healthy, total)
	}
	return "is-healthy", "All software up to date", fmt.Sprintf("%d/%d healthy", healthy, total)
}

func formatPortLabel(port int) string {
	if port > 0 {
...
func formatRootLabel(rootPath string) string {
	if trimmed := strings.TrimSpace(rootPath); trimmed != "" {
		return trimmed
	}
	return "Not configured"
}

func (h *ManagerHandlers) managerComponentHealth() (int, int, int) {
	...
	outdated := make(map[manager.DeployType]struct{}, total)
	...
	if outdatedCount > total {
		outdatedCount = total
	}
	healthy := total - outdatedCount
	if healthy < 0 {
		healthy = 0
	}
	return total, healthy, outdatedCount
}
...
func (h *ManagerHandlers) buildDashboardCardRequest(c *gin.Context) (*cards.Request, gin.H) {
	payload := h.buildDashboardPayload(c)
	if payload == nil {
		return nil, nil
	}
	req := &cards.Request{
...
func (h *ManagerHandlers) serverStatusPayload(c *gin.Context, s *models.Server, username interface{}, errMsg string) gin.H {
	...
	cardReq, payload := builder(c, s, username, errMsg)
	if payload == nil {
		return nil
	}
	payload["cardIDs"] = []string{}
	renderables := cards.BuildRenderables(cards.ScreenServerStatus, cardReq)
	if grouped := cards.GroupRenderablesBySlot(renderables); len(grouped) > 0 {
		payload["cardSlots"] = grouped
	}
...
func (h *ManagerHandlers) Dashboard(c *gin.Context) {
	cardReq, data := h.buildDashboardCardRequest(c)
	if data == nil {
		data = gin.H{}
	}
	if cardReq != nil {
		renderables := cards.BuildRenderables(cards.ScreenDashboard, cardReq)
		if grouped := cards.GroupRenderablesBySlot(renderables); len(grouped) > 0 {
...
func (h *ManagerHandlers) ManagerGET(c *gin.Context) {
	cardReq, data := h.buildManagerCardRequest(c)
	if data == nil {
		data = gin.H{}
	}
	if cardReq != nil {
		renderables := cards.BuildRenderables(cards.ScreenManager, cardReq)
		if grouped := cards.GroupRenderablesBySlot(renderables); len(grouped) > 0 {
			data["cardSlots"] = grouped
		}
		if ids := cards.RenderableIDs(renderables); len(ids) > 0 {
...
// ManagerCardGET renders a single manager card for HTMX refresh requests.
func (h *ManagerHandlers) ManagerCardGET(c *gin.Context) {
	if h.manager == nil {
	...
	cardReq, _ := h.buildManagerCardRequest(c)
	if cardReq == nil {
		c.String(http.StatusInternalServerError, "card context unavailable")
		return
	}

	renderable, ok := cards.BuildRenderableByID(cards.ScreenManager, cardID, cardReq)
	if !ok {
...
// DashboardCardGET renders a single dashboard card for HTMX refresh requests.
func (h *ManagerHandlers) DashboardCardGET(c *gin.Context) {
	if h.manager == nil {
		c.String(http.StatusServiceUnavailable, "manager unavailable")
		return
	}
	...
	cardReq, _ := h.buildDashboardCardRequest(c)
	if cardReq == nil {
		c.String(http.StatusInternalServerError, "card context unavailable")
		return
	}

	renderable, ok := cards.BuildRenderableByID(cards.ScreenDashboard, cardID, cardReq)
	if !ok {
		c.String(http.StatusNotFound, "card not found")
		return
	}
