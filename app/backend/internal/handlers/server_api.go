...
func (h *ManagerHandlers) APIServerStatus(c *gin.Context) {
	...
	if toggles := cloneCardToggleMap(s.CardToggles); len(toggles) > 0 {
		resp["card_toggles"] = toggles
	}
...
func (h *ManagerHandlers) APIServerUpdateSettings(c *gin.Context) {
	...
	// Apply card toggle updates when provided (UI form or JSON payload)
	snapshotSelections, snapshotProvided, snapshotUsedJSONMap := parseCardToggleSnapshotInput(jsonBody, c.Request.PostForm)
	if snapshotProvided {
		applyServerCardToggleSnapshot(s, snapshotSelections)
	}
	if partial := parseCardTogglePartialInput(jsonBody, snapshotUsedJSONMap); len(partial) > 0 {
		applyServerCardTogglePartial(s, partial)
	}
...
func (h *ManagerHandlers) APIServersCreate(c *gin.Context) {
	...
	var req struct {
		...
		CardToggles               map[string]bool `json:"card_toggles"`
	}
	var (
		formToggleSelections map[string]bool
		formToggleProvided   bool
		initialCardToggles   map[string]bool
	)
	if strings.Contains(strings.ToLower(c.GetHeader("Content-Type")), "application/json") {
		if err := c.ShouldBindJSON(&req); err != nil {
		...
		initialCardToggles = filterServerStatusCardToggles(req.CardToggles)
	} else {
		...
		if sel, provided, _ := parseCardToggleSnapshotInput(nil, c.Request.PostForm); provided {
			formToggleSelections = sel
			formToggleProvided = true
		}
	}

	if formToggleProvided {
		tmpServer := &models.Server{}
		applyServerCardToggleSnapshot(tmpServer, formToggleSelections)
		initialCardToggles = cloneCardToggleMap(tmpServer.CardToggles)
	}
	...
	cfg := &models.ServerConfig{
		...
		CardToggles: initialCardToggles,
	}
...
func (h *ManagerHandlers) APIServersCreateFromSave(c *gin.Context) {
	...
	var (
		cardToggleSelections map[string]bool
		cardToggleProvided   bool
		cardToggleConfig     map[string]bool
	)
	...
	for _, v := range c.PostFormArray("card_toggle") {
		toggleValues.Add("card_toggle", v)
	}
	if sel, provided, _ := parseCardToggleSnapshotInput(nil, toggleValues); provided {
		cardToggleSelections = sel
		cardToggleProvided = true
	}
	...
	if cardToggleProvided {
		tmpServer := &models.Server{}
		applyServerCardToggleSnapshot(tmpServer, cardToggleSelections)
		cardToggleConfig = cloneCardToggleMap(tmpServer.CardToggles)
	}
...
func parseCardToggleSnapshotInput(jsonBody map[string]any, form url.Values) (map[string]bool, bool, bool) {
	selections := make(map[string]bool)
	snapshot := false
	jsonMapConsumed := false

	if form != nil && len(form) > 0 {
		if strings.TrimSpace(form.Get("card_toggle_present")) != "" {
			snapshot = true
		}
		for _, raw := range form["card_toggle"] {
			id := normalizeCardToggleID(raw)
			if id == "" {
				continue
			}
			snapshot = true
	...
	if jsonBody != nil {
		if rawList, ok := jsonBody["card_toggle"]; ok {
			for _, raw := range stringSliceFromInterface(rawList) {
				id := normalizeCardToggleID(raw)
				if id == "" {
					continue
				}
		...
		} else if _, hasPresent := jsonBody["card_toggle_present"]; hasPresent {
			if toggleMap := parseCardToggleMapInput(jsonBody["card_toggles"]); len(toggleMap) > 0 {
				for id, enabled := range toggleMap {
					selections[id] = enabled
				}
				jsonMapConsumed = true
			}
			snapshot = true
		}
	}
	...
	return selections, true, jsonMapConsumed
}

func parseCardTogglePartialInput(jsonBody map[string]any, skipJSONMap bool) map[string]bool {
	if jsonBody == nil || skipJSONMap {
		return nil
	}
	return parseCardToggleMapInput(jsonBody["card_toggles"])
}

func parseCardToggleMapInput(value any) map[string]bool {
	if value == nil {
		return nil
	}
	...
	case map[string]any:
		for key, raw := range typed {
			if id := normalizeCardToggleID(key); id != "" {
				if enabled, ok := boolFromInterface(raw); ok {
					result[id] = enabled
				}
			}
		}
...
func allowedServerStatusCardIDs() map[string]struct{} {
	...
	for _, opt := range options {
		if id := normalizeCardToggleID(opt.ID); id != "" {
			ids[id] = struct{}{}
		}
	}
	if len(ids) == 0 {
...
func applyServerCardToggleSnapshot(s *models.Server, selections map[string]bool) {
	if s == nil {
		return
	}
	allowed := allowedServerStatusCardIDs()
	if len(allowed) == 0 {
		return
	}
	next := make(map[string]bool, len(s.CardToggles)+len(allowed))
	for id, enabled := range s.CardToggles {
		norm := normalizeCardToggleID(id)
		if norm == "" {
			continue
		}
...
func applyServerCardTogglePartial(s *models.Server, updates map[string]bool) {
	...
	if len(s.CardToggles) == 0 {
		s.CardToggles = make(map[string]bool, len(updates))
	}
...
func boolFromInterface(value any) (bool, bool) {
	switch v := value.(type) {
	case bool:
		return v, true
	case string:
...
func stringSliceFromInterface(value any) []string {
	switch v := value.(type) {
	case []any:
		list := make([]string, 0, len(v))
		for _, entry := range v {
			if s := normalizeCardToggleID(fmt.Sprint(entry)); s != "" {
				list = append(list, s)
			}
		}
		return list
	case []string:
		list := make([]string, 0, len(v))
		for _, entry := range v {
			if s := normalizeCardToggleID(entry); s != "" {
				list = append(list, s)
			}
		}
		return list
	case string:
		if s := normalizeCardToggleID(v); s != "" {
			return []string{s}
		}
		return nil
	default:
...
func cloneCardToggleMap(src map[string]bool) map[string]bool {
	if len(src) == 0 {
		return nil
	}
	dst := make(map[string]bool, len(src))
	for id, enabled := range src {
		if norm := normalizeCardToggleID(id); norm != "" {
			dst[norm] = enabled
		}
	}
	if len(dst) == 0 {
...
func filterServerStatusCardToggles(input map[string]bool) map[string]bool {
	if len(input) == 0 {
		return nil
	}
	allowed := allowedServerStatusCardIDs()
	...
	filtered := make(map[string]bool, len(input))
...
func normalizeCardToggleID(id string) string {
