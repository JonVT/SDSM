...
func copyCardToggleMap(src map[string]bool) map[string]bool {
	if len(src) == 0 {
		return nil
	}
	dst := make(map[string]bool, len(src))
	for cardID, enabled := range src {
		normalized := normalizeCardToggleKey(cardID)
		if normalized == "" {
			continue
		}
		dst[normalized] = enabled
	}
	if len(dst) == 0 {
		return nil
	}
...
func normalizeCardToggleKey(cardID string) string {
...
type ServerConfig struct {
	...
	PlayerSaves bool
	// CardToggles overrides per-card visibility (card ID ➝ enabled?).
	CardToggles map[string]bool
...
type Server struct {
	...
	// CardToggles overrides per-card visibility (card ID ➝ enabled?).
	CardToggles map[string]bool `json:"card_toggles,omitempty"`
...
func NewServerFromConfig(serverID int, paths *utils.Paths, cfg *ServerConfig) *Server {
	...
	s.CardToggles = copyCardToggleMap(cfg.CardToggles)
...
func NewServer(serverID int, paths *utils.Paths, data string) *Server {
	...
	s.CardToggles = copyCardToggleMap(s.CardToggles)
...
// IsCardEnabled reports whether the specified card should render for this server after
// evaluating server-specific overrides. When no override exists the card defaults to enabled.
func (s *Server) IsCardEnabled(cardID string) bool {
	if s == nil {
		return true
	}
	...
	if enabled, ok := s.CardToggles[id]; ok {
		return enabled
	}
	return true
}
