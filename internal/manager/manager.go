...
func (m *Manager) SCONLatest() string {
	...
	m.sconLatestMu.Lock()
	prevLatest := m.sconLatest
	m.sconLatest = version
	m.sconLatestAt = time.Now()
	m.sconLatestMu.Unlock()

	if err == nil && prevLatest != version {
